...
| standalone `-ctx`: every call into a required module is emitted as `Context::fnByMangledName` + `das_invoke_function` (`daslib/aot_cpp.das` `isHybridCall`, `func._module != program.getThisModule`) although the context defines the callee as an `inline` function in the same unit | 1734 lookup sites in the port's context, `nextOpImpl` and every helper an operation calls; no C++ inlining across them and no tail jump between operations | **not accepted upstream or in the fork** (found 2026-09-21 on master `1969ad4d4`, handed to the fork the same day, review pending). `notes/upstream_cases/ctx_direct_calls.patch` calls the emitted foreign functions directly, issue text `notes/upstream_cases/ctx_direct_calls.md`. The port applies it itself: `scripts/build_port.sh ctx` emits from `tmp/dasroot-ctx`, an overlay of DASLANG_ROOT whose `daslib/` copy carries every `ctx_*.patch` the checkout lacks (a marker string per patch; `WASM3DAS_CTX_EMITTER=stock` is the rollback). The owner's working tree has this one applied uncommitted, so the overlay currently applies only the second | the build prints the lookup count of the emitted unit: 310 with the patch, 1734 stock; `fib 35` on the ctx tier 1.65 s -> 1.15 s alone and 0.44 s with the C form of the operation ABI, spec 17863/17863, WASI 12/12 (`docs/native-build.md`, section 7) |
