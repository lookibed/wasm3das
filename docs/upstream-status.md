...
| LLVM JIT: every pointer dereference is preceded by a null test unless the function carries the `unsafeDeref` flag (`[unsafe_deref]`; `visitExprAt`, `visitExprPtr2Ref`) | three tests per executed operation (`_pc`, the slot pointer, the advanced `_pc`) that C does not have | not a defect, a facility, with one known hazard: the fork's issue lookibed/daScript#7 (reported 2026-09-23 by the c2das work) is a real miscompile under `[unsafe_deref]`, a nested void function whose only effect is a store through `reinterpret<uint8?>(uint64)` is dropped in every mode. The port's `m3_exec_expand` pass sets `fn.flags.unsafeDeref` on the operation path only (the `op_*` functions and the dispatch helpers, E11), none of which has that shape; the spec suite (17863/17863) and the fixture corpus (86/86) run with the flag on every tier | the DLL's operation body has no `jit_exception` branch left; re-check the #7 reproducer against the port's operations after every daslang bump |
