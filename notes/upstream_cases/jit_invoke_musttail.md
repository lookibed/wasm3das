...
Until then the port is measuring a port-side stand-in (an index word in the code page and a generated tree of direct calls, branch `perf/jit-dispatch-tree`): LLVM tail-jumps through it, but the central tree costs a prologue and nine comparisons per operation where the `-ctx` build pays one indirect jump.
