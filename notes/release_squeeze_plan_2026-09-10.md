...
| A6 | Release builds have no cache: three cold daslang builds per tag (8–20 min each) | `release.yml` run for v0.3.0 | minutes instead of half an hour per release | `actions/cache` keyed like the gate's (pin + `build-daslang.sh` hash) | done in PR #42: a warm release run is about 10 minutes end to end |
