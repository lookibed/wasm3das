...
stage_compile() {
    ...
    done < <(find source tests/integration app host_test -type f -name '*.das' -print | sort)
}

stage_lint() {
    local profile="$1"
    echo "gate: lint --$profile"
    "$DASLANG" "$DASLANG_ROOT/utils/lint/main.das" -- --"$profile" source tests/integration app host_test
}
